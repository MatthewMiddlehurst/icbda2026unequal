...
from .transforms.pad import Padder
from .transforms.resize import Resizer
from .transforms.truncate import Truncator
from .transforms.sfa import SFA
...
class TemporalDictionaryEnsemble(BaseClassifier):
    ...
    def _fit(self, X, y, keep_train_preds=False):
        ...
        max_window_searches = self.max_n_timepoints_ / 4
        max_window = int(self.max_n_timepoints_ * self.max_win_len_prop)
    ...
    def _unique_parameters(self, max_window, win_inc):
        possible_parameters = [
            [win_size, word_len, normalise, levels, igb, True if uq == 1 else False, True if uq == 2 else False]
...
class IndividualTDE(BaseClassifier):
    ...
    def _fit(self, X, y):
        ...
        if self.resize == True:
            self._r = Resizer()
            X = self._r.fit_transform(X)
        if self.pad == True:
            rng = check_random_state(self.random_state)
            self._t = Truncator(truncated_length="max", error_on_short=False)
            self._p = Padder(add_noise=0.001, random_state=rng)
            X = self._t.fit_transform(X)
            X = self._p.fit_transform(X)
    ...
    def _predict(self, X):
        ...
        if self.resize == True:
            X = self._r.transform(X)
        if self.pad == True:
            X = self._t.transform(X)
            X = self._p.transform(X)
