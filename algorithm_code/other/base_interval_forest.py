...
from ..transforms.random_intervals import RandomIntervals
...
class BaseIntervalForest(ABC):
    ...
    def _fit_forest(self, X, y, save_transformed_data=False):
        ...
        if self.base_estimator is None:
            # if _base_estimator exists from implementing class use it as the default
            # base estimator
            if getattr(self, "_base_estimator", None) is not None:
                pass
        ...
        max_series_lengths = [max([series.shape[1] for series in t]) for t in Xt]
                ...
                if isinstance(method, int):
                    ...
                    if method.lower() == "sqrt":
                        self._n_intervals[i] += int(
                            np.sqrt(max_series_lengths[i]) * np.sqrt(t[0].shape[0])
                        )
        ...
        # of the series length)
            ...
            self._min_interval_length = [
                int(self.min_interval_length * max_series_lengths[i] for i in range(len(Xt)))
            ]
        ...
        # list values must be ints or floats. The same checks as above are performed
            ...
            for i, length in enumerate(self.min_interval_length):
                if isinstance(length, float) and length <= 1:
                    self._min_interval_length.append(int(length * max_series_lengths[i]))
        ...
        for i, n in enumerate(self._min_interval_length):
            if n > max_series_lengths[i]:
                self._min_interval_length[i] = max_series_lengths[i]
            elif n < 3:
        ...
        # of the series length)
            ...
            self._max_interval_length = [
                int(self.max_interval_length * max_series_lengths[i]) for i in range(len(Xt))
            ]
        # if the input is a list, it must be the same length as the number of
        # series_transformers
        # list values must be ints or floats. The same checks as above are performed
            ...
            for i, length in enumerate(self.max_interval_length):
                if isinstance(length, float) and length <= 1:
                    self._max_interval_length.append(int(length * max_series_lengths[i]))
                elif isinstance(length, int):
        ...
        for i, n in enumerate(self._max_interval_length):
            ...
            elif n > max_series_lengths[i]:
                self._max_interval_length[i] = max_series_lengths[i]
    ...
    def _predict_for_estimator(self, Xt, estimator, intervals, predict_proba=False):
        interval_features = np.empty((len(Xt[0]), 0))
