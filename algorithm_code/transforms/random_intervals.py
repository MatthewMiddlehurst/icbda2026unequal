...
class RandomIntervals(BaseCollectionTransformer):
    ...
    def _fit_setup(self, X):
        ...
        elif self.max_interval_length > self.max_n_timepoints_:
            self._max_interval_length = self.max_n_timepoints_
    ...
    def _generate_interval(self, X, y, seed, transform):
        ...
        if rng.random() < 0.5:
            interval_start = (
                rng.randint(0, self.max_n_timepoints_ + 1 - self._min_interval_length)
                if self.max_n_timepoints_ > self._min_interval_length
                else 0
            )
            len_range = min(
                self.max_n_timepoints_ + 1 - interval_start,
        ...
        if uq:
            g = [
                np.expand_dims(x[dim, round(
                    interval_start * (x.shape[1] / self.max_n_timepoints_)):round(
                    interval_end * (x.shape[1] / self.max_n_timepoints_)):dilation],
                               axis=0)
                for x in X
            ] if isinstance(X, list) else np.expand_dims(
                X[:, dim, interval_start:interval_end:dilation], axis=1)

            p = [True for _ in range(len(g))]
            for l in range(len(g)):
                if g[l].shape[1] < self._min_interval_length:
                    g[l] = np.zeros((1, self._min_interval_length))
                    p[l] = False
        else:
            g = [
                np.expand_dims(x[dim, interval_start:interval_end:dilation], axis=0)
                for x in X] if isinstance(X, list) else np.expand_dims(
                X[:, dim, interval_start:interval_end:dilation], axis=1)
            p = [True for _ in range(len(g))]
            for l in range(len(g)):
                if g[l].shape[1] == 0:
                    ...
                    pad = rng.uniform(0, 0.001, size=interval_length - g[l].shape[1])
                    g[l] = np.concatenate((g[l], np.expand_dims(pad, axis=0)), axis=1)
        ...
        for feature in self._features:
            if isinstance(feature, BaseTransformer):
                ...
                t = [
                    [feature(f[0]) if p[i] else np.nan]
                    for i, f in enumerate(g)
                ]
                Xt = np.hstack((Xt, t))

            intervals.append((interval_start, interval_end, dim, feature, dilation, uq))

        return intervals, Xt

    def _transform_interval(self, X, idx, seed, keep_transform):
        interval_start, interval_end, dim, feature, dilation, uq = self.intervals_[idx]
        interval_length = interval_end - interval_start
        rng = check_random_state(seed)
